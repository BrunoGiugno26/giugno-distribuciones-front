...
const HIDDEN_CATEGORY_SLUGS = [
  ...
  "accesorios-herramientas-pro",
];

const PROFESSIONAL_ROUTES: Record<string, string> = {
  "muebles-pro": "/peluquerias/muebles",
};
  ...
  const visibleCategories = Array.isArray(result)
    ? result.filter(
        (category: CategoryType) =>
          !HIDDEN_CATEGORY_SLUGS.includes(category.attributes.slug),
      )
    : [];
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        {!loading &&
          visibleCategories.map((category: CategoryType) => {
            ...
            const finalHref = PROFESSIONAL_ROUTES[slug] ?? `category/${slug}`;
            return (
              <Link
                key={category.id}
                href={finalHref}
